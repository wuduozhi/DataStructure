...
void RBTree<T>::removeFixUp(RBTNode<T>* &root, RBTNode<T> *node, RBTNode<T> *parent){
    ...
    while((!node || rb_is_black(node)) && node!=root){
        if(parent->left == node){
            other = parent->right;
            if(rb_is_red(other)){  //Case 1: x的兄弟节点w是红色的
                rb_set_black(other);
                rb_set_red(parent);
                leftRotate(root,parent);
                other = parent->right;
            ...
            if((!other->left || rb_is_black(other))&&(!other->right||rb_is_black(other->right))){
                //Case 2: x的兄弟节点w是黑色的，且w的两个孩子也是黑色的
                rb_set_red(other);
                node = parent;
                parent = rb_parent(node);
            }else{
                if(!other->right || rb_is_black(other->right)){
                    //Case 3:x的兄弟w是黑色的，且w的左节点是红色的，右节点是黑色de
                ...
                //Case 4:x的兄弟节点w是黑色的，且w的右孩子是红色的，左孩子任意颜色
...
int main()
{
    int a[]= {10, 40, 30, 60, 90, 70, 20, 50, 80,110,100,130,140,120,150};
    ...
    cout << "== The max: " << tree->minimum() << endl;
    cout << "== The min: " << tree->maximum() << endl;
    cout << "== The tree: " << endl;
    tree->print();
